package com.softilys.myapp.web.rest;
...
import com.softilys.myapp.IntegrationTest;
import com.softilys.myapp.config.EmbeddedKafka;
import com.softilys.myapp.config.KafkaSseConsumer;
import com.softilys.myapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class EditiqueSampleApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/editique-sample-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/editique-sample-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/editique-sample-application-kafka/unregister"));
                return;
            }
        }
